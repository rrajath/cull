...
| Sentry | Crash reporting | Wired via `io.sentry.android.gradle`; releases tagged with the semver `versionName` so crashes map back to a specific release. Privacy-constrained: screenshots, view-hierarchy attachment, and interaction breadcrumbs are disabled (a crash screenshot in a photo app is almost certainly a personal photo); traces sampled at 10%. |
...
- Build types: `debug` sets `applicationIdSuffix = ".debug"` (`com.rrajath.cull.debug`), `resValue("string", "app_name", "Cull Debug")`, and `versionNameSuffix = " (debug)"`, so debug and release install side by side with separate data. `buildFeatures.resValues = true` is required because AGP 9 disables it by default. UI that shows the app name reads `R.string.app_name` (manifest label, home wordmark) so it picks up the debug name. `release` is unchanged.
- `versionName` is hand-maintained semver (`appVersionName` in `app/build.gradle.kts`); `versionCode` is derived from it as `MAJOR * 10000 + MINOR * 100 + PATCH` (1.2.3 -> 10203), and configuration fails if the name isn't `MAJOR.MINOR.PATCH` or MINOR/PATCH exceed 99. Deriving the code keeps the two from drifting and keeps it monotonic across semver bumps. The same version is the Sentry release identifier and the release tag (`printVersionName` exposes it to CI).
- `.github/workflows/build.yaml` runs on every push to `master` and only validates: `./gradlew assembleRelease assembleDebug` with no keystore (release falls back to debug signing), nothing published.
- `.github/workflows/release.yml` runs on `v*.*.*` tag pushes: verifies the tag equals `v<versionName>` and is on `master`, builds both variants (release signed from a base64-encoded keystore secret that is removed unconditionally afterward; missing secret fails the run), renames APKs to `$APP_NAME-v<version>-<variant>.apk` (`APP_NAME: cull`), publishes a GitHub Release with the CHANGELOG `Unreleased` entries as notes, then moves those entries into a `## [<version>] - <date>` section and pushes it to `master` as `github-actions[bot]`. The move is skipped with a warning if `master` has advanced past the tag, so later entries are never attributed to the wrong version. Changelog parsing lives in `.github/scripts/changelog.py` (testable locally) rather than inline awk. Actions are pinned to full commit SHAs, not mutable tags.
