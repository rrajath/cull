...
- `.github/workflows/build.yaml` runs on every push to `master`: full-history checkout (needed for the commit-count version), JDK 17 setup, `./gradlew assembleRelease assembleDebug` (release is R8-minified, signed from a base64-encoded keystore secret, cleaned up unconditionally afterward), a rename step that gives both APKs the CI-only name `$APP_NAME-v<version>-<variant>.apk` (`APP_NAME: cull` is a workflow-level env var), then a GitHub Release tagged `v<version>` with both APKs and auto-generated notes. Actions are pinned to full commit SHAs, not mutable tags.
